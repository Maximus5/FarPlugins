...
//
void DumpObjRelocations( MPanelItem *pRoot, WORD machine, PIMAGE_RELOCATION pRelocs, DWORD count )
{
    DWORD i;
    char szTypeName[32];
    ...
    for ( i=0; i < count; i++ )
    {
        ...
        pRoot->printf("  Address: %08X  SymIndex: %08X  Type: %s\n",
...
// COFF OBJ file.
//
bool DumpObjFile( MPanelItem *pRoot, PIMAGE_FILE_HEADER pImageFileHeader )
{
    unsigned i;
    PIMAGE_SECTION_HEADER pSections;
    
    DumpHeader(pRoot, pImageFileHeader);
    pRoot->printf("\n");

    pSections = MakePtr(PIMAGE_SECTION_HEADER, (pImageFileHeader+1),
                            pImageFileHeader->SizeOfOptionalHeader);

    DumpSectionTable(pRoot, pSections, pImageFileHeader->NumberOfSections, FALSE);
    pRoot->printf("\n");
    ...
    if ( fShowRelocations )
    {
        for ( i=0; i < pImageFileHeader->NumberOfSections; i++ )
        {
            ...
            pRoot->printf("Section %02X (%.8s) relocations\n", i+1, pSections[i].Name);
            DumpObjRelocations(	pRoot, pImageFileHeader->Machine,
    ...
    if ( fShowLineNumbers )
    {
        ...
        for (i=0; i < pImageFileHeader->NumberOfSections; i++)
        {
            ...
            {
                DumpLineNumbers( pRoot, MakePtr(PIMAGE_LINENUMBER, pImageFileHeader,
    ...
    {
        DumpRawSectionData( pRoot, (PIMAGE_SECTION_HEADER)(pImageFileHeader+1),
                            pImageFileHeader,
